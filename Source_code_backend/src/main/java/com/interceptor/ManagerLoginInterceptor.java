...
public class ManagerLoginInterceptor implements HandlerInterceptor {
    ...
    public String refreshToken(String token, HttpServletResponse response, HttpServletRequest request) {
        ...
        try {
            ...
            jwtEntityManager = (JwtEntityManager) JSONObject.toJavaObject(jsonObject, JwtEntityManager.class);

            HttpSession session = request.getSession(); 


            Manager manager = managerService.getById(jwtEntityManager.getManagerid());

            session.setAttribute("managerid", manager.getManagerid());
            session.setAttribute("username", manager.getUsername());
            session.setAttribute("realname", manager.getRealname());
        ...
        if ((expire - currentTime) / 1000 < 300) {
            ...
            token = JwtUtil.getJwtToken(jwtEntityManager, JwtUtil.DEFAULT_EXPIRE);
