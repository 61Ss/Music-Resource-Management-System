    ...
    <div>
        ...
        >
            ...
            <el-form-item label="Category" prop="projectinfo_typeid">
                <el-select size="small" v-model="postForm.projectinfo_typeid" placeholder="Select">
                    <el-option
            ...
            <el-form-item label="Name" prop="projectinfoname">
                <el-input placeholder="Enter" v-model="postForm.projectinfoname"/>
            </el-form-item>

            <el-form-item label="Keywords" prop="keyword_table">
                <el-input style="width: 290px" placeholder="Enter" v-model="postForm.keyword_table"/>
            </el-form-item>
            <el-form-item label="Image" prop="image">
                <Upload v-model="postForm.image" accept=".jpg,.png,.gif"/>
            </el-form-item>
            <el-form-item label="Attachments" prop="projectinfo_files">
                <Upload v-model="postForm.projectinfo_files" accept=".mp3"/>
            </el-form-item>
            <el-form-item label="Top Status" prop="projectinfo_istop">
                <el-radio-group v-model="postForm.projectinfo_istop">
                    <el-radio value='Yes' key='Yes'>Yes</el-radio>
                    <el-radio value='No' key='No'>No</el-radio>
                </el-radio-group>

            </el-form-item>
            <el-form-item label="Carousel" prop="projectinfo_isflv">
                <el-radio-group v-model="postForm.projectinfo_isflv">
                    <el-radio value='Yes' key='Yes'>Yes</el-radio>
                    <el-radio value='No' key='No'>No</el-radio>
                </el-radio-group>
...
const projectinfo_typeOptions = ref({});//这里写注释
...
function fetchData() {
    if (props.dataId > 0) {
        //显示编辑数据
        axios_request.get('/manager/projectinfo/getById.action?id=' + props.dataId).then(response => {
                if (response.data) {
        ...
        //显示添加数据
        axios_request.get('/manager/projectinfo/create.action').then(response => {
                if (response.data) {

                    projectinfo_typeOptions.value = response.data.projectinfo_typeList
                    postForm.value.projectinfo_typeid = response.data.projectinfo_typeList[0].projectinfo_typeid// 首次打开,默认选中第一个
    ...
    postFormRef.value.validate(valid => {
        ...
        if (valid) {
            ...
            if (props.dataId > 0) {
                ...
                axios_request.post('/manager/projectinfo/update.action', postForm.value).then(response => {
                ...
                axios_request.post('/manager/projectinfo/add.action', postForm.value).then(response => {
                    ElMessage({message: response.message})
                    if (response.message) {
                        closeDialog()//必须在axios异步过程内
                    }
